            ...
            <div className='max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8'>
                <div className="alert alert-info shadow-lg text-white">
                    <div>
                        ...
                        <span>Untuk saat ini silahkan gunakan menubar yang ada dibagian atas website ini untuk mengakses menu fitur yang tersedia.</span>
                    </div>
                </div>
                ...
                <div className="flex flex-col justify-center items-center w-full gap-6 mt-5">
                    ...
                    <div className='flex flex-col justify-center items-center'>
                        <form onSubmit={submit}>
                            <input className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-sky-50 file:text-sky-500hover:file:bg-violet-100" type="file" name="image" onChange={e => setData(e.target.files)} />
                            {progress && (
                                <progress value={progress.percentage} max="100">
                                    {progress.percentage}%
                                </progress>
                            )}
                            <button type="submit" disabled={processing} className='btn text-white w-full rounded-md mt-2 bg-sky-600 hover:bg-sky-700'>Upload Avatar</button>
                        </form>
                    </div>
                </div>
                <div className='flex flex-col justify-center items-center mt-3'>     
                    <form onSubmit={updateUsername}>
                        <div className="form-group float-left">
                            ...
                            <input onChange={handleChangeUsername} name='username' type="text" className="form-control input input-bordered input-info w-full max-w-xs" value={username} />
                        </div>
                        <button type='submit' disabled={processing} className='btn  text-white  bg-sky-600 hover:bg-sky-700 float-right  mt-6 ml-2'>Simpan</button>
                    </form>
                </div>
            </div>
        </Authenticated>
