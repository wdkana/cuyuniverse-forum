          ...
          <div className='flex flex-col justify-center items-center'>
            ...
            <div className='flex flex-col justify-center items-center'>
              <form onSubmit={updatePhoto} className="py-3">
                <input className="block w-full text-lg file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary hover:file:bg-secondary file:cursor-pointer" type="file" name="image" onChange={e => setData(e.target.files)} />
                {progress && (
                  <progress value={progress.percentage} max="100">
                    {progress.percentage}%
                  </progress>
                )}
                <button type="submit" disabled={processing} className='btn w-full border-0 rounded-md mt-2 bg-primary hover:bg-secondary'>Upload Avatar</button>
              </form>
            </div>
          </div>
