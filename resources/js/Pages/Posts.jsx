...
export default function PostsPage(props) {
  ...
  const [keyword, setKeyword] = useState(filter.search ? filter.search : "");

  const reload = useCallback(
    debounce(q => {
      Inertia.get("/posts", pickBy({search: q, page: filter.page, filtered: filter.filtered}), {
        preserveState: true,
      });
    }, 500),
    []
                ...
                <Menu.Items className="absolute w-52 z-50 bg-white dark:bg-slate-600 shadow rounded-lg mt-1.5 overflow-hidden py-0.5 text-gray-900 dark:text-slate-100">
                  {menus.map((menu, key) => {
                    return menu.name == "line" ? (
                      <div key={key} className="h-px bg-gray-200 my-0.5 w-full"></div>
                    ) : (
                      <Menu.Item key={key}>
                        <Link
                          preserveState
