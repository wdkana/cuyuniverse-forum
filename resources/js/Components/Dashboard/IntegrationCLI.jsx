  ...
  return (
    <div>
      ...
      <div className="card p-6 lg:card-side bg-base-100 shadow-xl">
        <figure>
          <div className="max-w-md mockup-code bg-secondary text-secondary-content">
            <pre data-prefix="1"><code>cuy login dea</code></pre>
            <pre data-prefix="2"><code>authenticating..</code></pre>
            <pre data-prefix="3" className="bg-warning text-warning-content"><code>cuycli activated</code></pre>
          </div>
        </figure>
        <div className="card-body">
          <div className="flex justify-end">
            <label className="btn btn-link" htmlFor="cliGuide" onClick={() => setIsCLIGuide(true)}>panduan penggunaan</label>
          </div>
          <div className="pb-4">
            <h2 className="card-title">Aktivasi CUY-CLI</h2>
            <p>Lakukan aktifitas lewat terminal kesayangan anda.</p>
            {cli ?
              <pre className="pt-2 mb-4">
          ...
          <div className="card-actions justify-end">
            <label className="btn btn-primary" htmlFor={cli ? "cliOff" : "cliOn"} onClick={() => setIsCLIRequest(true)}>{cli ? "Matikan" : "Aktifkan"}</label>
          </div>
        </div>
      </div>
    </div>
      ...
      <label htmlFor={cli ? "cliOff" : "cliOn"} className="modal cursor-pointer">
        <label className="modal-box relative" htmlFor={cli ? "cliOff" : "cliOn"}>
          ...
          {!cli &&
            <div>
              <p className="py-4">
                CUYCLI adalah command line tools untuk memudahkan anda melakukan aktifitas tanpa perlu membuka website cuyuniverse.
              </p>
              <p className="pb-4">
                Semua kegiatan dilakukan via terminal desktop anda secara langsung.
              </p>
            </div>
          }
          <div className="flex justify-end">
            <button className="btn btn-primary" onClick={() => requestActivatingCLI(!cli)}>{cli ? "Ya, Matikan" : "Ya, Aktifkan"}</button>
          </div>
        </label>
      </label>
    </>
    ...
    <>
      ...
      <label htmlFor="cliGuide" className="modal cursor-pointer">
        <label className="modal-box relative" htmlFor="cliGuide">
          ...
          <div>
            <p className="py-4">
              Integrated CMDer for cuyuniverse platform
            </p>
          </div>
          <div className="flex justify-end">
            <button className="btn btn-primary" onClick={() => setIsCLIGuide(false)}>MENGERTI</button>
          </div>
        </label>
      </label>
    </>
